...
const AppModules = {
    ...
    initGlobalFixedMenu() {
        ...
        // 获取所有按钮（包括没有菜单的实时数据按钮）
        const menuBtns = document.querySelectorAll('.left-fixed-btn-group button');
        // 获取所有二级菜单面板
        ...
        menuBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                // 实时数据按钮：没有二级菜单，直接切换到实时模式
                if (btnId === 'showRealtimeDataBtn') {
                    // 关闭所有菜单和时间轴
                    closeAllMenus();
                    closeTimeline();
                    
                    // 清除其他模式按钮的 mode-active
                    menuBtns.forEach(b => b.classList.remove('mode-active'));
                    btn.classList.add('mode-active');
                    
                    // 退出其他模式（历史模式、预测模式等）
                    if (window.App && window.App.exitHistoryMode) {
                ...
                const isMenuActive = targetMenu?.classList.contains('active') || false;
                ...
                // ===== 历史数据/预测模式已激活时再次点击按钮 =====
                if (isModeActive && isCalendarBtn) {
                    // 如果点击的是不同的日历按钮（历史↔预测），则切换模式
                    const otherBtnId = btnId === 'showHistoryBtn' ? 'predictTrafficBtn' : 'showHistoryBtn';
                    const otherBtn = document.getElementById(otherBtnId);
                    if (otherBtn && otherBtn.classList.contains('mode-active')) {
                        // 另一个日历按钮也激活了，说明是从另一种模式切换过来，需要先退出
                        if (window.App && window.App.stopAllVisualizations) {
                            window.App.stopAllVisualizations();
                        }
                        otherBtn.classList.remove('mode-active');
                        ...
                        if (btnId === 'showHistoryBtn') {
                            ...
                            if (window.App) {
                                ...
                                if (window.App.updateModeLabel) {
                                    window.App.updateModeLabel();
                                }
                            }
                        } else if (btnId === 'predictTrafficBtn') {
                            // 切换到预测模式
                            if (window.App) {
                                window.App.predictionModeActive = true;
                                if (window.App.updateModeLabel) {
                                    window.App.updateModeLabel();
                                }
                            }
                ...
                if (isModeActive) {
                    ...
                    if (btnId === 'realtimePlanBtn') {
                        if (window.App && window.App.exitPlanMode) {
                            window.App.exitPlanMode();
                        }
                    } else if (btnId === 'customPathBtn') {
                        // 自定义路径模式：退出时清理选择状态
                        window.isPathSelectMode = false;
                        window.pathSelectedNodes = [];
                        const cpBar = document.getElementById('pathConfirmBar');
                        if (cpBar) cpBar.style.display = 'none';
                    } else if (btnId === 'customizeEventsBtn') {
                        // 突发事件模式：退出时清理选点状态
                        window.isNodeSelectMode = false;
                        const nodeBtn = document.getElementById('nodeSelectBtn');
                        if (nodeBtn) nodeBtn.classList.remove('active');
                        if (window.clearEmergencyHighlights) window.clearEmergencyHighlights();
                    } else {
                        // 实时数据/自定义：退出模式
                        if (window.App && window.App.stopAllVisualizations) {
                            window.App.stopAllVisualizations();
                ...
                if (isCalendarBtn) {
                    ...
                    if (btnId === 'showHistoryBtn') {
                        ...
                        // 更新顶部显示为历史模式
                        if (window.App && window.App.updateModeLabel) {
                            window.App.updateModeLabel();
                        }
                    } else if (btnId === 'predictTrafficBtn') {
        ...
        document.querySelectorAll('#planMenu .submenu-item').forEach(item => {
        ...
        document.addEventListener('click', (e) => {
            ...
            // 如果点击目标在地图上，且处于选点模式，不关闭菜单
            const mapEl = document.getElementById('map');
            if (mapEl && mapEl.contains(e.target) && (window.isPathSelectMode || window.isNodeSelectMode)) return;
        ...
        if (exitHistoryBtn) {
            exitHistoryBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                if (window.App && window.App.exitHistoryMode) {
                    window.App.exitHistoryMode();
                }
                ...
                const predictBtn = document.getElementById('predictTrafficBtn');
                if (historyBtn) historyBtn.classList.remove('mode-active');
                if (predictBtn) predictBtn.classList.remove('mode-active');
            });
        }
    },
    ...
    selectDate(dateStr, type) {
        if (type === 'history') {
            ...
            console.log(`[UI] ========== 开始模型预测流程 ==========`);
            console.log(`[UI] 选择预测日期: ${dateStr}`);
            console.log(`[UI] window.App 存在: ${!!window.App}`);
            console.log(`[UI] window.App.enterPredictMode 存在: ${typeof window.App?.enterPredictMode === 'function'}`);
        
            // ===== 新增：预测模式 - 显示顶部信息栏 =====
            const historyInfoBar = document.getElementById('historyInfoBar');
            if (historyInfoBar) {
                // 更新信息栏文本（区分预测模式）
                historyInfoBar.querySelector('.mode-label').textContent = '模型预测模式';
                historyInfoBar.querySelector('.date-label').textContent = `预测日期：${dateStr}`;
                // 显示信息栏
                historyInfoBar.classList.remove('hide');
                historyInfoBar.classList.add('show');
            }
        
            // 通过主应用暴露的接口进入预测模式，确保模式状态一致
            if (window.App && window.App.enterPredictMode) {
                console.log(`[UI] 调用 window.App.enterPredictMode('${dateStr}')`);
                window.App.enterPredictMode(dateStr);
            } else {
                console.error('[UI] 主应用未挂载 enterPredictMode 方法');
                console.error('[UI] window.App:', window.App);
            }
            console.log(`[UI] ========== 模型预测流程调用结束 ==========`);
        }
        ...
        // 时间轴：历史模式和预测模式下保持显示，其他模式下关闭
        const timelineEl = document.querySelector('.timeline-container');
        if ((type !== 'history' && type !== 'predict') && timelineEl.classList.contains('active')) {
            timelineEl.classList.add('closing');
            timelineEl.addEventListener('animationend', () => {
                timelineEl.classList.remove('active', 'closing');
            }, { once: true });
        ...
        // 历史模式和预测模式下确保时间轴可见
        if ((type === 'history' || type === 'predict') && timelineEl) {
            timelineEl.classList.add('active');
            timelineEl.classList.remove('closing');
        }
    },
